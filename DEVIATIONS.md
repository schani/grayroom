...
| Loupe zoom | done | The loupe is the develop canvas, read-only: `CanvasNSView` with the pan tool, fed for a developed photo by the *same render loop* the develop view uses. So `0`/`1`, the double-click toggle, pinch, ⌘-scroll and scroll-to-pan are not a second implementation, they are the same one, and `d`/`e` between the two views re-decode nothing — the loupe points its canvas at the frame the loop already holds. What is still Lightroom's and not here: the navigator rectangle, and the 2:1/4:1 steps above 100 %. |
| Loupe fills the window | deviation | Lightroom's `E` keeps the left panel. Grayroom's collapses it: one photo, the whole window. The panel's own visibility is untouched on the way in, so `g` comes back to it exactly as it was left, hidden or showing. |
| Loupe of an undeveloped photo | deferred | "As developed" is development #1 run through the pipeline. A photo with no development shows the camera's own embedded preview instead of a default-parameter render — the same rule the grid follows (`PreviewBuilder`), and what makes `e` instantaneous; a default-parameter render would also be *black and white*, which is not what an unedited frame looks like anywhere else in the app. It goes on the same canvas, so it pans and zooms identically, but with no pipeline over it and therefore no EDR. The cost is that a camera which embeds a small preview (640 px on one of the Phase One files here) is magnified to fill the window — the canvas fits the *photo's* frame and the small texture stands in for it, exactly as the develop view's draft pass does. Rendering every undeveloped RAW at loupe size is a full decode per photo the user is only flicking past. |
